...
class Bank:
    ...
    def get_account(self, account_number):
        if account_number not in self.accounts:
            raise AccountNotFoundError("Счет с таким номером не найден.")
...
# Снятие средств
try:
    bank.withdraw_from_account("12345", 5033)
    bank.withdraw_from_account("12345", 120330)  # Эта операция вызовет исключение InsufficientFundsError
